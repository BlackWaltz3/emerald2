 ...
 * Task_NewGameBirchSpeech_StartBirchRaltsPlatformFade
 * Task_NewGameBirchSpeech_StartBirchRaltsPlatformFade
 ...
 * Task_NewGameBirchSpeech_ReshowBirchRalts
 ...
 *  - Advances to Task_NewGameBirchSpeechSub_WaitForRalts
 * Task_NewGameBirchSpeechSub_WaitForRalts
...
static void Task_NewGameBirchSpeechSub_WaitForRalts(u8);
static void Task_NewGameBirchSpeech_StartBirchRaltsPlatformFade(u8);
...
static void Task_NewGameBirchSpeech_ReshowBirchRalts(u8);
...
static const struct MenuAction sMenuActions_Gender[] = {
    {COMPOUND_STRING("He/Him"), {NULL}},
    {COMPOUND_STRING("She/Her"), {NULL}}
};

static const u8 *const sMalePresetNames[] = {
    COMPOUND_STRING("Emmett")
};
...
{
    u8 spriteId = gTasks[sBirchSpeechMainTaskId].tRaltsSpriteId;
    ...
    CreatePokeballSpriteToReleaseMon(spriteId, gSprites[spriteId].oam.paletteNum, 112, 58, 0, 0, 32, PALETTES_BG, SPECIES_RALTS);
    gTasks[taskId].func = Task_NewGameBirchSpeechSub_WaitForRalts;
    gTasks[sBirchSpeechMainTaskId].tTimer = 0;
}

static void Task_NewGameBirchSpeechSub_WaitForRalts(u8 taskId)
{
    s16 *data = gTasks[taskId].data;
    struct Sprite *sprite = &gSprites[gTasks[sBirchSpeechMainTaskId].tRaltsSpriteId];
...
static void Task_NewGameBirchSpeech_AndYouAre(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        ...
        AddTextPrinterForMessage(TRUE);
        gTasks[taskId].func = Task_NewGameBirchSpeech_StartBirchRaltsPlatformFade;
    }
}

static void Task_NewGameBirchSpeech_StartBirchRaltsPlatformFade(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
        gSprites[gTasks[taskId].tRaltsSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
...
static void Task_NewGameBirchSpeech_StartPlayerFadeIn(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        gSprites[gTasks[taskId].tRaltsSpriteId].invisible = TRUE;
        if (gTasks[taskId].tTimer)
        {
            gTasks[taskId].tTimer--;
        }
...
static void Task_NewGameBirchSpeech_StartNamingScreen(u8 taskId)
{
    ...
    {
        FreeAllWindowBuffers();
        FreeAndDestroyMonPicSprite(gTasks[taskId].tRaltsSpriteId);
...
static void Task_NewGameBirchSpeech_SlidePlatformAway2(u8 taskId)
{
    ...
    {
        gTasks[taskId].func = Task_NewGameBirchSpeech_ReshowBirchRalts;
    }
}

static void Task_NewGameBirchSpeech_ReshowBirchRalts(u8 taskId)
{
    u8 spriteId;
    ...
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        gSprites[spriteId].invisible = FALSE;
        gSprites[spriteId].oam.objMode = ST_OAM_OBJ_BLEND;
        spriteId = gTasks[taskId].tRaltsSpriteId;
        gSprites[spriteId].x = 100;
        gSprites[spriteId].y = 75;
        gSprites[spriteId].invisible = FALSE;
        gSprites[spriteId].oam.objMode = ST_OAM_OBJ_BLEND;
...
static void Task_NewGameBirchSpeech_WaitForSpriteFadeInAndTextPrinter(u8 taskId)
{
    ...
    {
        gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_NORMAL;
        gSprites[gTasks[taskId].tRaltsSpriteId].oam.objMode = ST_OAM_OBJ_NORMAL;
        if (!RunTextPrintersAndIsPrinter0Active())
        {
            gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
            gSprites[gTasks[taskId].tRaltsSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
...
static u8 NewGameBirchSpeech_CreateRaltsSprite(u8 x, u8 y)
{
    return CreateMonPicSprite_Affine(SPECIES_RALTS, FALSE, 0, MON_PIC_AFFINE_FRONT, x, y, 14, TAG_NONE);
}
...
static void AddBirchSpeechObjects(u8 taskId)
{
    ...
    lotadSpriteId = NewGameBirchSpeech_CreateRaltsSprite(100, 0x4B);
    gSprites[lotadSpriteId].callback = SpriteCB_Null;
    gSprites[lotadSpriteId].oam.priority = 0;
    gSprites[lotadSpriteId].invisible = TRUE;
    gTasks[taskId].tRaltsSpriteId = lotadSpriteId;
